...
<\body>
  <tmdoc-title|boot.scm>

  <\scm-chunk|boot.scm|false|false>
    \;
    ...
    ;; DESCRIPTION : some global variables, public macros, on-entry, on-exit
    and

    ;; \ \ \ \ \ \ \ \ \ \ \ \ \ \ initialization of the TeXmacs module
    system
    ...
    \ \ \ \ \ \ (tm-output (object-\<gtr\>string (car l)))))
    ...
    \ \ \ \ \ \ \ \ \ \ (and (pair? head) (symbol? (car head)) (not (defined?
    (car head)))))
    ...
    \ \ \ \ \ \ ;; \ \ \ \ `(module-use! (current-module) (resolve-module
    ',module)))
    ...
    \ \ \ \ \ \ \ \ \ \ ((equal? (car action) :use) (cons 'use-modules (cdr
    action)))

    \ \ \ \ \ \ \ \ \ \ ((equal? (car action) :inherit) (cons
    'inherit-modules (cdr action)))

    \ \ \ \ \ \ \ \ \ \ ((equal? (car action) :export)

    \ \ \ \ \ \ \ \ \ \ \ (display "Warning] The option :export is no longer
    supported\\n")

    \ \ \ \ \ \ \ \ \ \ \ (display " \ \ \ \ \ \ ] Please use tm-define
    instead\\n"))
    ...
    \ \ \ \ \ \ \ \ (set! l (cons `(module-use! (current-module)
    ,texmacs-user) l)))
