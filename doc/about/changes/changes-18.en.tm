...
<\body>
  ...
  operator in a subtraction <math|x-y>, there is are small spaces around the
  minus sign <math|->; this is no longer the case in <math|-x>, where we
  using it as a<nbsp>prefix. Similarly, the spacing inside lists of operators
  <math|+,-,\<times\>> is now correct. However, the modification may modify
